...
export function SiteShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="relative min-h-screen bg-[#020806] text-white selection:bg-emerald-300 selection:text-emerald-950">
      ...
      <div className="pointer-events-none fixed inset-0 bg-[radial-gradient(circle_at_50%_0%,rgba(16,185,129,0.12),transparent_40%),radial-gradient(circle_at_90%_70%,rgba(20,184,166,0.06),transparent_35%)]" />

      <header className="sticky top-4 z-50 mx-auto max-w-6xl px-4 sm:px-6">
        <div className="glass-nav flex items-center justify-between rounded-full px-5 py-3 shadow-lg">
              ...
              return (
                <Link
                  ...
                  className={`relative rounded-full px-4 py-2 transition ${
                    isActive
                      ? "font-semibold text-emerald-300 bg-emerald-300/10 border border-emerald-300/30"
          ...
          <div className="flex items-center gap-3">
            <Link
              href="/"
              className="hidden rounded-full border border-emerald-300/30 bg-emerald-300/10 px-4 py-2 text-xs font-semibold text-emerald-300 transition hover:bg-emerald-300 hover:text-emerald-950 sm:inline-block"
            >
              Explore Orbit
            </Link>
            ...
            <button
              ...
              className="grid h-9 w-9 place-items-center rounded-full border border-white/10 bg-white/5 text-white/80 transition hover:border-emerald-300/40 md:hidden"
                ...
                return (
                  <Link
                    ...
                    className={`rounded-xl px-4 py-2.5 transition ${
                      isActive
                        ? "bg-emerald-300/15 font-semibold text-emerald-300 border border-emerald-300/30"
