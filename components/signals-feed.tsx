...
export function SignalsFeed() {
  ...
  return <div className="mt-10 grid gap-6 lg:grid-cols-[minmax(0,1fr)_21rem]">
    <section>
      ...
      <div className="mt-6 flex flex-wrap gap-2" aria-label="Filter signals">{filters.map((item) => <button key={item} type="button" aria-pressed={filter === item} onClick={() => setFilter(item)} className={`rounded-full px-4 py-2 text-xs font-semibold transition ${filter === item ? "bg-emerald-300 text-emerald-950" : "border border-white/10 bg-white/[0.03] text-white/55 hover:border-emerald-300/45 hover:text-emerald-200"}`}>{item === "All" ? "All signals" : item}</button>)}</div>
    ...
    <aside aria-live="polite" aria-atomic="true" className="h-fit rounded-2xl border border-white/10 bg-white/[0.03] p-6 lg:sticky lg:top-6">
